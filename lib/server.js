...
function loadOverlay(cb) {
    let val = '';
    metadata.getUUID(logger, (err, uuid) => {
        logger.info('## Manage me at http://127.0.0.1:3000/register/'+uuid);
        const url = 'http://' + _config.reportEndpoint + '/api/v1/config/overlay/' + uuid;
        http.get(url, (res) => {
            res.on('data', (chunk) => {
                val += chunk;
            }).on('end', () => {
...
function patchConfiguration(newConf) {
    ...
    _config.authData.accounts = accounts;
    _config.overlayVersion = newConf.version;

    _config.emit('authdata-update');
}

function reportInterval() {
    const to = http.request({
        method: 'POST',
        host: _config.reportEndpoint,
        path: '/api/v1/report',
        headers: {
...
class S3Server {
    ...
    startup(port, ipAddress) {
        ...
        setInterval(reportInterval, 5000);
    }
